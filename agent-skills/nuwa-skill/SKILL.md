...
```
Phase 0   入口分流 → 明确人名(0A) / 模糊需求(0B诊断→推荐→选择)
Phase 0.5 创建Skill目录结构
Phase 1   6个并行Agent多源信息采集
Phase 1.5 🔴 CHECKPOINT：调研质量摘要 → 用户确认
Phase 2   框架提炼（心智模型/启发式/表达DNA/价值观/谱系/边界）
Phase 2.5 🔴 CHECKPOINT：提炼摘要 → 用户确认
Phase 3   按模板构建SKILL.md
Phase 4   🔴 CHECKPOINT：质量验证（已知/边缘/风格测试）→ 用户确认
Phase 5   双Agent精炼（自动后置工序）
```
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查当前 runtime 的 skills 目录）
...
**来源A：本地已有Skill**
扫描当前 runtime 的 skills 目录中 `*-perspective/` 子目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
[skills-directory]/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
- 质量自检（Phase 4用）：逐项检查以下6项通过标准，输出PASS/FAIL：
  - 心智模型数量（3-7个，每个有来源证据）
  - 每个模型的局限性（明确写出失效条件）
  - 表达DNA辨识度（读100字能认出是谁）
  - 诚实边界（至少3条具体局限）
...
| 场景 | 触发条件 | 处理动作 |
|------|---------|---------|
| 单个Agent超时 | 搜索5分钟无有价值结果 | 不等待，继续推进。Phase 2标注「信息不足」，诚实边界中说明 |
| 信息源匮乏 | <10条可用来源 | Phase 0.5提醒用户，心智模型减至2-3个，增加诚实边界篇幅 |
| Agent结果冲突 | 两个Agent对同一事实给出矛盾结论 | 保留矛盾——用「内在张力」section收录 |
| 本地语料格式不支持 | 用户提供的文件无法解析（加密PDF、音频无字幕等） | 告知用户具体哪个文件无法处理，建议转换格式或提供替代素材 |
| Phase 5 双Agent精炼失败 | 子agent超时或两份报告建议互相矛盾 | 跳过精炼，保留Phase 4通过的版本交付，告知用户「精炼步骤跳过」 |
| 网络搜索全部失败 | 所有搜索工具不可用或返回空 | 如有本地语料→切换为纯本地模式；无本地语料→终止并告知用户 |

**关键规则**：宁可生成一个诚实标注了局限的60分Skill，也不要生成一个看起来完美但实际上在编造的90分Skill。

### Phase 1.5: 调研Review检查点

**🔴 CHECKPOINT · 🛑 STOP：所有Agent完成后，暂停展示调研质量摘要**：
...
1. **扫描**：从 `02-conversations.md` 和 `05-decisions.md` 中提取所有「如果X，则Y」模式的判断规则
2. **筛选**：保留有具体案例支撑的（此人在真实场景中用过），丢弃纯理论表述
3. **去重**：与心智模型重叠的合并（启发式是模型的操作化表达）
4. **数量控制**：取5-10条。超过10条 → 按使用频率排序取top 10；不足5条 → 从心智模型推导补充，标注「推导」

**记录格式**：每条含——触发场景（什么情况下用）、规则（如果X，则Y）、真实案例（此人何时用过）

#### 2.3 表达DNA分析

**数据来源**：主要从 `03-expression-dna.md`（碎片表达）+ `02-conversations.md`（即兴对话）提取。

**逐维度提取**：

| 维度 | 提取内容 | 提取方法 |
|------|---------|---------|
| 句式偏好 | 长句/短句、疑问/陈述、类比密度 | 统计10段典型文本的平均句长和类比频率 |
| 词汇特征 | 高频词、专属术语、禁忌词 | 从碎片表达中提取出现≥5次的特征词 |
| 节奏感 | 先结论还是先铺垫、转折方式 | 分析5段长回答的结构模式 |
| 幽默方式 | 讽刺/自嘲/荒诞/冷幽默/不幽默 | 收集所有幽默实例，归类主导风格 |
| 确定性表达 | 「我不确定」型 还是 「很明显」型 | 统计确定性词汇 vs 对冲词汇的比例 |
| 引用习惯 | 爱引谁、引什么类型 | 列出所有引用，按被引人/类型分组 |

**输出格式**：每个维度给出1-2句结论 + 2-3个原文例证。最终转化为Skill中的「风格规则」（供角色扮演时遵循）。
...
**🔴 CHECKPOINT · 🛑 STOP：Phase 2提炼完成后，暂停展示提炼摘要给用户确认**：
...
#### Step 4: 输出
将完成的SKILL.md写入 `[skills-directory]/[person-name]-perspective/SKILL.md`。
...
**🔴 CHECKPOINT · 🛑 STOP：展示验证结果给用户确认后才算完成。**
...
| # | 反模式 | 为什么危险 | 正确做法 |
|---|--------|-----------|---------|
| 1 | 编造此人没说过的话 | 破坏Skill可信度，用户无法区分真实框架和AI编造 | 所有心智模型必须有≥2个来源证据 |
| 2 | 把通用道理包装成「独特见解」 | 「要努力」「要专注」不是任何人的独特框架 | 三重验证：跨域复现+生成力+排他性 |
| 3 | 忽略负面评价和争议 | 只呈现正面=偶像化，不是蒸馏 | Agent 4 专门收集批评和外部视角 |
| 4 | 在信息不足时强行生成 | 宁可60分诚实Skill，不要90分编造Skill | 信息<10条时降级+扩大诚实边界 |
| 5 | 过度角色扮演（模仿语气说废话） | Skill是认知框架，不是cosplay | 表达DNA服务于传递思维方式，不是表演 |
| 6 | 用训练语料代替实时搜索 | LLM记忆可能过时或失真 | Phase 1 必须用工具搜索，不可跳过 |
| 7 | 一次性输出不经检查点确认 | 方向错误时返工成本极高 | 3个检查点（1.5/2.5/4）强制暂停 |
| 8 | 把多个人的观点混入单人Skill | 污染该人物的独特性 | 每个论点追溯到具体来源，不可混用 |
