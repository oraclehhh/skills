...
**操作步骤**：
1. **算白痴指数**：成品价格 / 原材料成本 = ？
2. **定位溢价层**：指数 > 5 时，逐层拆解供应链，找出溢价最高的环节
3. **评估自建可行性**：该环节的技术壁垒是什么？自建需要多少初始投入？回收期多长？
4. **决策**：白痴指数 > 5 且技术壁垒可攻克 → 垂直整合；白痴指数 < 3 或壁垒过高 → 外包
...
**操作步骤**：
1. **设定激进时间线**：比「合理」估计压缩50%，制造紧迫感（但内心知道这是目标不是承诺）
2. **定义最小可失败版本**：能验证核心假设的最简原型是什么？先做这个
3. **失败后提取学习**：每次失败必须回答「学到了什么？下一版改什么？」
4. **判断可逆性**：失败是可逆的（火箭炸了再造）→ 继续迭代；失败不可逆（涉及人命/法律）→ 放慢，不适用本模型
...
本Skill基于以下来源提炼（详细调研记录见 `references/` 目录）：
...
→ 详见 [`references/马斯克即兴思考方式调研.md`](references/马斯克即兴思考方式调研.md)
...
→ 详见 [`references/马斯克决策模式与行为分析-20260404.md`](references/马斯克决策模式与行为分析-20260404.md)

**思想体系综合**：→ 详见 [`references/Elon-Musk-思想体系调研-20260404.md`](references/Elon-Musk-思想体系调研-20260404.md)
