...
ensure_built() {
  ...
  if [ ! -f "$SKILL_DIR/dist/daemon.mjs" ]; then
    ...
    # Use ls + stat instead of find — find is extremely slow on Windows (MINGW)
    local bundle="$SKILL_DIR/dist/daemon.mjs"
    local newer
    newer=$(ls "$SKILL_DIR"/src/*.ts "$SKILL_DIR"/src/**/*.ts 2>/dev/null | while read -r f; do
      [ "$f" -nt "$bundle" ] && echo 1 && break
    done)
    if [ -n "$newer" ]; then
      need_build=1
    fi
    if [ "$need_build" = "0" ] && [ -d "$SKILL_DIR/node_modules/claude-to-im/src" ]; then
      newer=$(ls "$SKILL_DIR"/node_modules/claude-to-im/src/*.ts "$SKILL_DIR"/node_modules/claude-to-im/src/**/*.ts 2>/dev/null | while read -r f; do
        [ "$f" -nt "$bundle" ] && echo 1 && break
      done)
      if [ -n "$newer" ]; then
        need_build=1
...
case "$(uname -s)" in
  Darwin)
    ...
    ;;
  MINGW*|MSYS*|CYGWIN*)
    # Windows via Git Bash — use Linux-style nohup (avoids PowerShell spawn issues)
