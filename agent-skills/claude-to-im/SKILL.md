...
name: Codex-to-im
description: |
  Bridge THIS Codex or Codex session to Telegram, Discord, Feishu/Lark, QQ, or WeChat so the
  user can chat with Codex from their phone. Use for: setting up, starting, stopping,
  or diagnosing the Codex-to-im bridge daemon; forwarding Codex replies to a messaging
...
The skill directory (SKILL_DIR) is at `~/.Codex/skills/Codex-to-im`.
In Codex installs it may instead be `~/.codex/skills/Codex-to-IM-skill`.
If neither path exists, fall back to Glob with pattern `**/skills/**/Codex-to-im/SKILL.md` or `**/skills/**/Codex-to-IM-skill/SKILL.md` and derive the root from the result.
...
|---|---|
| `setup`, `configure`, `配置`, `我想在飞书上用 Codex`, `帮我连接 Telegram`, `帮我接微信` | setup |
...
1. **Codex** — `AskUserQuestion` tool is available. Use it for interactive setup wizards.
...
- **If it does NOT exist:**
  - In Codex: tell the user "No configuration found" and automatically start the `setup` wizard using AskUserQuestion.
...
- **Runtime**: `Codex` (default), `codex`, `auto`
  - `Codex` — uses Codex CLI + Codex Agent SDK (requires `Codex` CLI installed)
  - `codex` — uses OpenAI Codex SDK (requires `codex` CLI; auth via `codex auth login` or `OPENAI_API_KEY`)
  - `auto` — tries Codex first, falls back to Codex if Codex CLI not found
