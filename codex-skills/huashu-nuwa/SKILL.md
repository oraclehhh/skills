...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查当前运行环境的 skills 根目录；Codex 默认 `C:\Users\16494\.codex\skills`）
...
**来源A：本地已有Skill**
扫描当前运行环境的 skills 根目录下的 `*-perspective/` 目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
<skills-root>/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
Phase 1启动前，**主动扫描当前运行环境的 skills 根目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比通用网页搜索更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `<skills-root>/[person-name]-perspective/SKILL.md`。本机当前默认 `<skills-root>` 为 `C:\Users\16494\.codex\skills`。
